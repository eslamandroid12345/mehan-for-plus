...
class ResidentSeekersService
{
    ...
    public function store($request) {
        ...
        try {
            ...
            DB::commit();
            return redirect()->route('resident-seekers.index')->with(['success' => __('messages.Seeker registered successfully')]);
        } catch (\Exception $e) {
            DB::rollBack();
            return redirect()->route('resident-seekers.create')->with(['error' => __('messages.Something went wrong')]);
        }
    }
    ...
    private function storeSeeker($request, $user) {
        $seeker = $this->seekerRepository->create([
            ...
            'is_resident' => 1,
            'residency_number' => !$this->nationalityRepository->isSaudi($request->nationality_id) ? $request->residency_number : null,
            'residency_expiration' => !$this->nationalityRepository->isSaudi($request->nationality_id) ? $request->residency_expiration : null,
    ...
    public function update($request, $id) {
        ...
        try {
            ...
            DB::rollBack();
            return redirect()->route('resident-seekers.edit', $id)->with(['error' => __('messages.Something went wrong')]);
        }
    }
