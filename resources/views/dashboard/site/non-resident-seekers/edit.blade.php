...
@section('title', __('titles.Edit Non-Resident Seeker'))
    ...
    <section class="content">
        <div class="container-fluid">
            <div class="row">
                <div class="col-12">
                    <div class="card">
                        <form action="{{ route('non-resident-seekers.update', $user->id) }}" method="post" autocomplete="off" enctype="multipart/form-data">
                            <div class="card-header">
                                <h3 class="card-title">@lang('dashboard.Edit Non-Resident Seeker')</h3>
                            </div>
                            <div class="card-body">
                                @csrf
                                @method('put')
                                ...
                                <div class="form-group">
                                    ...
                                    <select name="country_id" class="form-control select2 select2-hidden-accessible" style="width: 100%;">
                                        ...
                                        @foreach($countries as $country)
                                            <option @selected(!is_null(old('country_id')) ? old('country_id') == $country->id : $user->seeker->city->country_id == $country->id) value="{{ $country->id }}">{{ $country->t('name') }}</option>
                                        @endforeach
                                    </select>
                                </div>
                                <div class="form-group">
                                ...
                                <div class="form-group">
                                    ...
                                    <div class="row">
                                        <div class="custom-control custom-radio">
                                            <input class="custom-control-input custom-control-input-dark" type="radio" value="1" id="customRadio4" name="is_worked_before_in_ksa" @checked(!is_null(old('is_worked_before_in_ksa')) ? old('is_worked_before_in_ksa') == 1 : $user->seeker->is_worked_before_in_ksa == 1)>
                                            <label for="customRadio4" class="custom-control-label">@lang('dashboard.Yes')</label>
                                        </div>
                                        <div class="custom-control custom-radio">
                                            <input class="custom-control-input custom-control-input-dark custom-control-input-outline" type="radio" value="0" id="customRadio5" name="is_worked_before_in_ksa" @checked(!is_null(old('is_worked_before_in_ksa')) ? old('is_worked_before_in_ksa') == 0 : $user->seeker->is_worked_before_in_ksa == 0)>
                ...
                $.ajax({
                    ...
                    success: function(result){
                        ...
                        for ( var i = 0; i < result.data.length; i++ ) {
                            $('[name=city_id]').append('<option '+(result.data[i].id == {{(old('city_id')) ?? $user->seeker->city_id}} ? 'selected' : '')+' value='+result.data[i].id+'>'+result.data[i].name+'</option>');
                        }
                    },
                    error: function () {
                        $('[name=city_id]').html('');
